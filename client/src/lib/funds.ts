...
export function getPeerRank(fund: Fund, funds: Fund[]) { const peers=funds.filter((candidate)=>candidate.asset_class===fund.asset_class); const ordered=[...peers].sort((a,b)=>b.returns.five_year-a.returns.five_year); return {rank:ordered.findIndex((candidate)=>candidate.fund_id===fund.fund_id)+1,total:ordered.length}; }
...
export function downloadComparisonCsv(funds: Fund[], allFunds: Fund[], locale: Locale) {
  ...
  const blob = new Blob([csv], { type: "text/csv;charset=utf-8" }); const url = URL.createObjectURL(blob); const link = document.createElement("a"); link.href = url; link.download = `mpf-fund-compare-${funds.length}-funds-${funds[0]?.as_of || "data"}.csv`; document.body.appendChild(link); link.click(); link.remove(); URL.revokeObjectURL(url);
}
