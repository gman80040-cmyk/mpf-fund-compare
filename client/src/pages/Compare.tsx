...
export default function Compare() {
  const { document, loading } = useFunds(); const { locale } = useLocale(); const t = copy[locale];
  const storedIds = JSON.parse(localStorage.getItem(selectionKey) || "[]") as string[]; const sharedIds = new URLSearchParams(window.location.search).get("funds")?.split(",").filter(Boolean) || []; const ids = storedIds.length ? storedIds : sharedIds; const funds = document?.funds.filter((fund) => ids.includes(fund.fund_id)) || [];
