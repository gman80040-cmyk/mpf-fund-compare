...
export default function Home() {
  ...
  const [query, setQuery] = useState(""); const [category, setCategory] = useState<AssetClass | "all">("all");
  ...
  const filtered = useMemo(() => funds.filter((fund) => { const searchText = [fund.name["zh-Hant"], fund.name.en, fund.provider, fund.scheme["zh-Hant"], fund.scheme.en].join(" ").toLowerCase(); return (category === "all" || fund.asset_class === category) && searchText.includes(query.trim().toLowerCase()); }), [funds, query, category]);
